...
export class CompanyService {
  ...
  async create(createCompanyDto: CreateCompanyDto): Promise<Company> {
    ...
    const nbPoiNeededToValidateCompany = parseInt(this.configService.get<string>('NB_POI_NEEDED_TO_VALIDATE_COMPANY'));
    createdCompany.status = countPoi < nbPoiNeededToValidateCompany ? CompanyStatus.NotEnoughPoi : CompanyStatus.AccountSent;
    
    await createdCompany.save();
    return this.findByUuid(createdCompany.id);
  }

  async update(company: Company, updateCompanyDto: UpdateCompanyDto): Promise<Company> {
